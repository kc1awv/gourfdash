...
func urfdUptime(pidFile, checkFile string) (time.Duration, error) {
...
/*func main() {
	pidFile := "/var/run/urfd.pid"
	checkFile := "/var/log/urfd.xml"
	uptime, err := urfdUptime(pidFile, checkFile)
	if err != nil {
		fmt.Println(err)
		os.Exit(1)
	}
	fmt.Printf("urfd has been up for %s", uptime)
}
*/
